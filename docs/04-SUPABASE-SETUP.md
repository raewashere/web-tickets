...
> **Note:** After the trigger creates the profile, the Angular app is responsible for inserting the appropriate role into `user_roles` (`artist` for Admin App registrations, `customer` for Store App registrations).
